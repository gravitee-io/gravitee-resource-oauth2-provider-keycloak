...
public class OAuth2KeycloakResourceTest {
    ...
    public void shouldFailTokenExchange() throws Exception {
        ...
        stubFor(
            post(urlEqualTo(KEYCLOAK_TOKEN_URI)).willReturn(
                aResponse().withStatus(400).withBody("{\"error\":\"invalid_request\",\"error_description\":\"missing audience\"}")
            )
        );
        ...
        assertTrue(handler.getResponse().getThrowable().getMessage().contains("400"));
        assertTrue(handler.getResponse().getThrowable().getMessage().contains("invalid_request"));
        assertTrue(handler.getResponse().getThrowable().getMessage().contains("missing audience"));
    ...
    public void shouldFailTokenExchangeWhenSubjectTokenIsMissing() throws Exception {
        ...
        resource.tokenExchange(TokenExchangeRequest.builder(null, TokenExchangeRequest.TOKEN_TYPE_ACCESS_TOKEN).build(), handler);
        assertTrue(lock.await(10000, TimeUnit.MILLISECONDS));

        assertFalse(handler.getResponse().isSuccess());
        assertTrue(handler.getResponse().getThrowable().getMessage().contains("subject_token is required"));

        verify(0, postRequestedFor(urlEqualTo(KEYCLOAK_TOKEN_URI)));
    }
