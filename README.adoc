...
| 5.0 and upper  | 4.12.18 to latest | 21
| 4.0 and upper  | 4.12.x to 4.12.17 | 21
| 3.0 and upper  | 4.10.x to 4.11.x  | 17
| 2.0 and upper  | until 4.9.x       | 17
|===
...
When the API is secured by a JWT plan and the resource is only consumed by the
`OpenID Connect - UserInfo` policy, no client credentials are needed: the UserInfo
call authenticates with the caller's own access token. The `credentials` block of
the adapter configuration is only used to authorize introspection calls, and can
be omitted:
...
`resource` stays mandatory — the Keycloak adapter refuses to start without it,
with `Must set 'resource' in config` — but it is the client identifier, not a secret.
